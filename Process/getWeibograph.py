...
def loadEid(event, id, y):
    ...
    if len(event) < 2:
        return None
    if len(event) > 1:
        if os.path.exists(os.path.join(cwd, 'data/Weibo/Weibograph/' + id + '.npz')):
            pass
        else:
            # print('the id is ', id)
            ...
            rootfeat, tree, x_x, rootindex, y, senlen = root_feat.detach().numpy(), np.array(tree_), np.array(
                x_features), np.array(root_index), np.array(y), np.array(x_senlen)
            del x_features, x_senlen, tree_, root_feat, root_index
            gc.collect()
            np.savez(os.path.join(cwd, 'data/Weibo/Weibograph/' + id + '.npz'), x=x_x, root=rootfeat,
                     edgeindex=tree, rootindex=rootindex, y=y, seqlen=senlen)
            # with open(os.path.join(cwd, 'data/Weibo/Weibograph/' + id + '.npz')) as f:
            #
...
def main(obj):
    ...
    treePath = os.path.join(cwd, 'data/Weibo/Weibo/weibo_data_all.txt')
    print("reading tree")
    i = 0
    treeDic = {}
    for line in open(treePath):
        ...
        # if i==518163 or i==518162 or i==518164:
        #     print(line)
        # i+=1
        if not treeDic.__contains__(eid):
            treeDic[eid] = {}
        # treeDic[eid][indexC] = {'parent': indexP, 'time_delay': time_delay, 'text': text}
        treeDic[eid][indexC] = {'parent': indexP, 'text': text}
    print('tree no:', len(treeDic))

    labelPath = os.path.join(cwd, "data/Weibo/Weibo/weibo_label_all.txt")
