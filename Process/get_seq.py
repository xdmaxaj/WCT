...
def get_dep_seq(index,data_path=os.path.join('..', '..', 'data', 'Weibograph')):
...
def dep_seq2json(index, dep_node_path, save_path, treePath):
    treeDic = {}
    mark = 0
    treeDic[index] = {}
    ...
    seq_dic['node_seq'] = {}
    seq_dic['context_seq'] = {}
    for key in dep_node_path:
        seq_dic['node_seq']['dep'+key] = dep_node_path[key].copy()
        seq_dic['context_seq']['dep' + key] = {}
        for key_1 in dep_node_path[key]:
            node_list = dep_node_path[key][key_1]
            context_list = []
            for i in range(len(node_list)):
                context_list.append(treeDic[index][node_list[-i - 1] + 1])
            seq_dic['context_seq']['dep'+key][key_1] = context_list.copy()
...
def main():
    ...
    save_path = os.path.join(cwd, 'data', dataset_name, dataset_name.split('-')[0] + 'seq')
    ...
    for file in tqdm(npz_file):
        index = file.split('.')[0]
        dep_node_path = get_dep_seq(index, data_path=data_path)
        dep_seq2json(index, dep_node_path, save_path, treePath)
